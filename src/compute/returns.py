"""Daily, trailing, monthly, and annualised return calculations on cleaned inf_diario data."""
...
def daily_returns(df_clean: pd.DataFrame) -> pd.DataFrame:
    ...
    return df.loc[
        mask, ["cnpj", "subclass_id", "date", "nav", "return_daily"]
    ].reset_index(drop=True)


def _trailing_one_window(quotas: pd.DataFrame, months: int) -> pd.Series:
    """Compute V_last / V_start − 1 per fund for a single trailing window of ``months``."""
...
def trailing_returns(
    ...
    """Trailing point-to-point returns for each fund × window, indexed on (cnpj, subclass_id)."""
    quotas = ri[["nav"]].sort_index()
    parts = {
        f"return_{label}": _trailing_one_window(quotas, m)
...
def monthly_returns(
    ...
    Output columns: cnpj, subclass_id, month (datetime64[M]), r_fund, r_cdi.
    """
    cdi_monthly = (
